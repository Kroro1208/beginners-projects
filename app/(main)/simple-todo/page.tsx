                ...
                <div className="flex flex-col items-center p-8 bg-slate-500 shadow-2xl rounded-xl">
                    ...
                    <div className="w-full max-w-md mt-8">
                        <ul className="space-y-4">
                            {memo.map((item) => (
                                <li key={item.id} className="flex justify-between items-center bg-gradient-to-r from-orange-200 to-orange-300 p-4 rounded-lg shadow-lg transform hover:scale-105 transition-all duration-300">
                                    <span className={`flex-1 text-gray-800 ${item.isComplete ? 'line-through' : ''}`}>
                                        {item.isComplete ? "✔️ " : ""}{item.text}
                                    </span>
                                    <div className="flex space-x-2">
                                        <button type='button' onClick={() => completeMemo(item.id)}
                                        ...
                                        <button type='button' onClick={() => deleteMemo(item.id)}
